...
def _odeint(func, rtol, atol, mxstep, hmax, y0, ts, *args):
  ...
  def scan_fun_base(carry, target_t):
    ...
    step_costs, *carry = lax.while_loop(cond_fun, body_fun, [0] + carry)
    _odeint_fwd, _, t, _, last_t, interp_coeff = carry
    ...
    return carry, y_target, step_costs
  
  def scan_fun(carry, target_t):
      cnter, _carry = carry
      _carry, y_target, step_costs = scan_fun_base(_carry, target_t)
      return (cnter + step_costs * 6, _carry), y_target
  ...
  (total_nfe, _), ys = lax.scan(scan_fun, (1, init_carry), ts[1:])
  # y0.shape: [N, 3, 32, 32] -> after vmap: y0.shape: [3, 32, 32]
  # ys.shape: [nums, 3, 32, 32]
  sb = ys.shape[1]
  return jnp.concatenate((y0[None], ys, jnp.array(total_nfe).reshape((1,1)).repeat(sb,axis=1))) 
#   return jnp.concatenate((y0[None], ys)), jnp.array(total_nfe).reshape((1,1)).repeat(2, axis=0)
#   return {
#       'images': jnp.concatenate((y0[None], ys)), # []
#     'nfe': jnp.array(total_nfe).reshape((1,1)).repeat(2, axis=0)
#   }
