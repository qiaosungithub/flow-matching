...
class AttentionPool2d(nn.Module):
    ...
    def __init__(
        ...
        # )
        torch_conv1d_kernel1 = partial(nn.Conv,kernel_size=(1,),kernel_init=nn.initializers.variance_scaling(mode='fan_in',distribution='uniform',scale=1.0),bias_init=torch_bias_initializer(embed_dim))
        self.positional_embedding = nn.Embed(spacial_dim ** 2 + 1, embed_dim, embedding_init=nn.initializers.normal(stddev=1/math.sqrt(embed_dim)),rngs=rngs)
        # self.qkv_proj = conv_nd(1, embed_dim, 3 * embed_dim, 1)
        self.qkv_proj = torch_conv1d_kernel1(in_features=embed_dim, out_features=3 * embed_dim,rngs=rngs)
        # self.c_proj = conv_nd(1, embed_dim, output_dim or embed_dim, 1)
        assert output_dim > 0
        self.c_proj = torch_conv1d_kernel1(in_features=embed_dim, out_features=output_dim,rngs=rngs)
    ...
    def __call__(self, x):
        ...
        position_embd = self.positional_embedding(positions)
        x = x + position_embd
        x = self.qkv_proj(x)
        q, k, v = jnp.split(x, 3, axis=-1) # NLC
        ...
        weight = jnp.einsum("bihc,bjhc->bijh", q, k) / jnp.sqrt(c // self.num_heads)
        weight = jax.nn.softmax(weight, axis=2)
        x = jnp.einsum("bijh,bjhc->bihc", weight, v)
        x = x.reshape(b, -1, c)
        x = self.c_proj(x)
        return x[:, 0, :]
...
class NCSNppClassifier(nn.Module):
    """NCSN++ model, with only encoder (i.e. a classifier)"""
    ...
    def __init__(self,
        ...
        **kwargs
        ...
        assert resblock_updown, 'Default is True'
        nf = self.base_width = base_width
        self.image_size = image_size
        self.out_channels = out_channels
        self.ch_mult = ch_mult
        ...
        assert use_attn_pool, NotImplementedError
    ...
    def __call__(self, x, time_cond, augment_label=None,y=None, train=True, verbose=False): # turn off verbose here
        ...
        logging_fn(f"Input shape {x.shape}")
        name = f'enc_{cur_size}x{cur_size}_conv'  # 32x32_conv
        hs_fuyi = getattr(self, name)(x)
        # hs = [getattr(self, name)(x)]
        logging_fn(f"{name}: params {ps(name)}, shape {hs_fuyi.shape}") # 32x32xnf
        for i_level in range(self.num_resolutions):
            # Residual blocks for this resolution
            for i_block in range(self.num_res_blocks):
                name = f'enc_{cur_size}x{cur_size}_block{i_block}'
                assert temb.shape[-1] == 4 * nf
                h = getattr(self, name)(hs_fuyi, temb, train)
                logging_fn(f"{name}: params {ps(name)}, shape {h.shape}")
                # Here out_c is nf * ch_mult[i_level]
                assert h.shape[1] == cur_size
                if h.shape[1] in attn_resolutions:
...
if __name__ == "__main__":
    ...
    model = NCSNppClassifier(base_width=128, rngs=rngs, num_classes=10)
