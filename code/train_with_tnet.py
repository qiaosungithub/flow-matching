...
def train_step_compute(state: NNXTrainState, batch, noise_batch, t_batch, learning_rate_fn, ema_scales_fn, config, t_predictor_state=None):
  """
...
def sample_step(state, sample_idx, model, rng_init, device_batch_size, config,MEAN_RGB=None, STDDEV_RGB=None,option='FID',t_predictor_state=None):
  """
  ...
  images, nfe = generate(state, model, rng_sample, n_sample=device_batch_size,config=config,label_type=('order' if option=='vis' else 'random') if config.model.class_conditional else 'none',t_predictor_state=t_predictor_state)
...
def train_and_evaluate(
  ...
  if rank == 0 and config.wandb:
    wandb.init(project='LMCI', dir=workdir, tags=['Train_with_tpred'])
  ...
  ########### Create T predictor State ###########
  t_predictor_cls = models_ddpm.SimDDPM  
  t_predictor_rngs = nn.Rngs(config.seed, params=config.seed + 810, dropout=config.seed + 666, train=config.seed + 888)
  t_predictor_init_fn = partial(t_predictor_cls, num_classes=NUM_CLASSES, dtype=dtype)
  t_predictor = t_predictor_init_fn(rngs=t_predictor_rngs, **config.t_predictor_model)
  
  t_predictor_state = create_train_state(config, t_predictor, image_size, learning_rate_fn)
  assert config.get('load_t_predictor_from',None) is not None, 'Must provide a checkpoint path for evaluation'
  if not os.path.isabs(config.load_t_predictor_from):
    raise ValueError('Checkpoint path must be absolute')
  if not os.path.exists(config.load_t_predictor_from):
    raise ValueError('Checkpoint path {} does not exist'.format(config.load_t_predictor_from))
  t_predictor_state,_ = restore_checkpoint(t_predictor_init_fn, t_predictor_state, config.load_t_predictor_from, config.t_predictor_model, ema=False, is_classifier=True) # NOTE: whether to use the ema model
  
  print('T predictor loaded successfully!')
  ...
  p_train_step_compute = jax.pmap(
    partial(train_step_compute, 
            ...
            config=config, t_predictor_state=t_predictor_state),
    axis_name='batch'
  )
        ...
        if (step + 1) % config.log_per_step == 0:
          ...
          if config.wandb and index == 0:
            wandb.log({
              ...
              'lr': train_metrics['lr'], 
              't_predictor_diff': t_diff_to_display,
              'step': step, 
