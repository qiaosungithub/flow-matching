...
def train_step(state: NNXTrainState, batch, rngs, train_step_compute_fn, config):
  ...
  ## DDIM
  half = (b1 * b2) // 2 + 1
  t_batch = jax.random.randint(rngs.train(), (half, ), minval=0, maxval=config.model.num_diffusion_timesteps)
  t_batch = jnp.concatenate([t_batch, config.model.num_diffusion_timesteps - 1 - t_batch], axis=0)[:b1*b2]
...
def train_and_evaluate(
  ...
  if rank == 0 and config.wandb:
    wandb.init(project='LMCI', dir=workdir, tags=['SQA-DDIM-exp'])
