...
class SimDDPM(nn.Module):
  ...
  t_conditioned: bool = True

  C_1: float = 0.001 # iDDPM constant for u
  C_2: float = 0.008 # iDDPM constant for alpha_bar
  j_0: int = 8 # iDDPM constant for alpha_bar
  M: int = 1000 # iDDPM constant, number of discretization steps
  P_std: float = 1.2
  P_mean: float = -1.2
  ...
  def setup(self):
    ...
    js = np.arange(self.M + 1)
    alpha_bar = np.sin(0.5 * np.pi * js / self.M / (self.C_2 + 1)) ** 2
    u = np.zeros(self.M + 1)
    for j in range(self.M, 0, -1):  # num_timesteps, ..., 1
        u[j - 1] = np.sqrt(((u[j] ** 2 + 1) / (alpha_bar[j - 1] / alpha_bar[j]).clip(min=self.C_1) - 1))
    self.u = u  # km: shall we turn into jnp?
  ...
  def sample_one_step(self, x_i, rng, i, t_steps):
    ...
    # TODO: ema net?
    # Euler step.
    denoised = self.forward_iddpm_denoising_function(x_hat, t_hat, train=False)
  ...
  def compute_t(self, indices, scales):
    ...
    # -----------------------------------------------------------------
    # ticks = indices / (scales - 1)  # linear ticks
    ...
    # j = jnp.floor(self.j_0 + ticks * (self.M - 1 - self.j_0) + 0.5).astype(jnp.int32)
    # t = jnp.array(self.u)[j]
    # -----------------------------------------------------------------
  ...
  def forward_dbg_v1_denoising_function(self, x, sigma, augment_label=None, train: bool = True):  # EDM
    c_skip = self.sde.data_std ** 2 / (sigma ** 2 + self.sde.data_std ** 2)  # original
    # c_skip = 1. / (sigma ** 2 + 1e-3)
    # c_out = sigma * self.sde.data_std / jnp.sqrt(sigma ** 2 + self.sde.data_std ** 2)  # original
    c_out = jnp.ones_like(sigma)

    # c_in = 1 / jnp.sqrt(sigma ** 2 + self.sde.data_std ** 2)  # original
  ...
  def forward_iddpm_denoising_function(self, x, sigma, augment_label=None, train: bool = True):  # iDDPM
    
    # return self.forward_edm_denoising_function(x, sigma, augment_label, train)
    return self.forward_dbg_v1_denoising_function(x, sigma, augment_label, train)

    c_skip = jnp.ones_like(sigma)
    c_out = -sigma

    c_in = 1 / jnp.sqrt(sigma ** 2 + 1)
    if self.t_conditioned:
      # # First reshape sigma to (batch_size,) for easier broadcasting
      # sigma_flat = sigma.reshape(-1)  # shape: (batch_size,)

      # # Compute differences
      # diff = jnp.abs(self.u[:, None] - sigma_flat[None, :])  # shape: (M+1, batch_size)
      # j = jnp.argmin(diff, axis=0)  # shape: (batch_size,)

      # # Convert to time encoding
      # c_noise = self.M - 1 - j  # shape: (batch_size,)
  ...
  def forward(self, imgs, labels, augment_label, train: bool = True):
    ...
    data_std = 0.5
    weight = (sigma ** 2 + data_std ** 2) / (sigma * data_std) ** 2
    # weight = 1 / sigma ** 2
    # -----------------------------------------------------------------

    # -----------------------------------------------------------------
    # iDDPM noise recipe, reformulated in EDM:
    # j = jax.random.randint(self.make_rng('gen'), [bz, 1, 1, 1], 0, self.M)  # km: self.M or self.M - 1?
    # sigma = jnp.array(self.u)[j]
    # weight = 1 / sigma ** 2
    # -----------------------------------------------------------------
    ...
    xn = x + noise
    D_xn = self.forward_iddpm_denoising_function(xn, sigma, augment_label)
  ...
  def __call__(self, imgs, labels, train: bool = False):
    # initialization only
    t = jnp.ones((imgs.shape[0],)) if self.t_conditioned else None
